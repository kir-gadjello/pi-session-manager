...
Click any session to open it in the [Session Viewer](/docs/session-viewer).
...
Organizes sessions by their file system location — helpful when you have [multiple session paths](/docs/configuration) configured.

### Kanban View

A drag-and-drop board that organizes sessions into columns based on [tags](/docs/tags). See the dedicated [Kanban](/docs/kanban) page for details.
...
Select multiple sessions and export them in bulk. Supported formats: HTML, Markdown, JSON. See [Export](/docs/export) for details.
...
By default, sessions are scanned from `~/.pi/agent/sessions/`. You can add additional directories in [Configuration](/docs/configuration) to scan sessions from multiple locations (e.g., gateway sessions, project-specific paths).
