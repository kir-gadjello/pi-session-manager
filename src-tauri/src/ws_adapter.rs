...
pub async fn dispatch(app_state: &SharedAppState, command: &str, payload: &Value) -> Result<Value, String> {
    match command {
        "scan_sessions" => {
        ...
        "load_app_settings" => {
            crate::load_app_settings_internal().await
        }
        "save_app_settings" => {
        ...
        "terminal_create" => {
            ...
            manager.create_session(id, app_handle, event_tx, cwd, shell)?;
