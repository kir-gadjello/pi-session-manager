...
fn test_fts_migration_and_integrity() {
    ...
    let count_after_delete: i64 = conn.query_row(
...
fn test_backfill_when_message_entries_empty() {
    ...
    let triggers: Vec<String> = match conn.prepare("SELECT name FROM sqlite_master WHERE type='trigger' AND tbl_name='message_entries'") {
        ...
        Err(e) => { eprintln!("[DEBUG] trigger query error: {}", e); vec![] }
    };
    ...
    let fts_rows: Vec<(i64, String, String, String)> = match conn.prepare("SELECT rowid, session_path, role, content FROM message_fts") {
        Ok(mut stmt) => {
            stmt.query_map([], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?))).unwrap()
    ...
    let direct_matches_bin: Vec<i64> = match conn.prepare("SELECT rowid FROM message_fts WHERE message_fts MATCH 'binary*'") {
        Ok(mut stmt) => stmt.query_map([], |row| row.get(0)).unwrap().collect::<Result<Vec<_>, _>>().unwrap(),
        Err(e) => { println!("[DEBUG] MATCH 'binary*' error: {}", e); vec![] }
    };
    ...
    let direct_matches_tok: Vec<i64> = match conn.prepare("SELECT rowid FROM message_fts WHERE message_fts MATCH 'tokio*'") {
        Ok(mut stmt) => stmt.query_map([], |row| row.get(0)).unwrap().collect::<Result<Vec<_>, _>>().unwrap(),
        Err(e) => { println!("[DEBUG] MATCH 'tokio*' error: {}", e); vec![] }
    };
    ...
    assert!(results.iter().any(|(_, p, _, _, _, _)| p.contains("session1")),
        "Expected results to include session1; got paths: {:?}", results.iter().map(|r| &r.1).collect::<Vec<_>>());

    // Deletion cascade should still work
    conn.execute("DELETE FROM sessions WHERE path = ?", params![&sess2_path.to_string_lossy()]).unwrap();
    let me_remaining: i64 = conn.query_row(
        "SELECT COUNT(*) FROM message_entries WHERE session_path = ?",
